...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "crunchtabs", "tests")); system.time(test_check("crunchtabs", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran crunchtabs_$(VERSION).tar.gz
	rm -rf crunchtabs.Rcheck/

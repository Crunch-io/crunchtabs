...
This package was designed to allow for some elements of style and structure to pass between the two formats. However, it should not be assumed that if something works in one format it will work in another. In the rest of this document we have done our best to identify the formats that 
...
You can also append much larger chunks of text like a disclosure, analysis, or other marketing copy that is required for all of your documents. A typical workflow here is to create a .tex fragment file that includes your copy. Tex makes significant use of the `\` character. In R, you must _escape_ the backslash with an additional backslash. In the example below you seee `\begin` becomes `\\begin` and a new line `\\` becomes `\\\\`.
...
![Appending Arbitrary Tex](example-010-ct-append.png){width=650px} 

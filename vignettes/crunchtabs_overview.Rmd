...
Next, let's create a personal copy of the example data set for use with this vignette. This will ensure that you have the example dataset available to your account. A rudimentary understanding of the `crunch` R package is expected from readers of this document. Anyone with a `crunch` login should be able to follow along with the examples included here. If you're not familiar with `crunch`, we recommend the [getting started](https://crunch.io/r/crunch/articles/crunch.html) vignette.
...
![Topline Example from the Example Dataset](example-001-topline.png){width=650px}
...
![Cross Tabulation Example from the Example Dataset](example-002-crosstabs.png){width=650px}
...
![Cross Tabulation Excel Example from the Example Dataset](example-003-excel-ct.png){width=650px}
...
![Cross Tabulation Recode](example-004-ct-recode.png){width=650px}
...
![Cross Tabulation Relabel](example-005-ct-relabel.png){width=650px}
...
![Cross Tabulation Relabel](example-006-ct-weight.png){width=650px}
...
![Cross Tabulation Relabel](example-007-ct-moe.png){width=650px}

> NOTE: In Crunch, a "viewer" of a dataset does not have access to the individuals rows and cannot weight data or calculate margin of error using the R crunch package.

### Adding Titles

The default title behavior assigns the dataset name as the title on each page of the report. Often the internal reference used to denote a data set is not well aligned or appropriate for other stakeholder's. Below we will show you how to customize this output by using a title and a subtitle. 
...
![Including Margin of Error](example-008-ct-titles.png){width=650px}
...
![Cross Tabulation Relabel](example-009-ct-append.png){width=650px}

The string passed to `append_text` will be visible on the last page of your report. Appended text can also be much larger and can include paragraphs by using "\n" character as the `sep` argument in your `paste` function. 
...
writeLatex(
  ...
  filename = "output", 
  pdf = TRUE,
  append_text = paste0("Field Start: 2020-02-04", "Field End: 2020-03-04", sep = "\\\\")
) 
```
...
If you want to include a logo it should be in PNG format and should be of reasonable quality so as not to create scaling problems when users zoom into the document you have created. 
...
There are a large number of theme related options. In this section we will cover some of the more basic elements. 

...
#' @param vars A named list of vectors of aliases of the variables that should
...
#' @export
banner <- function(dataset, vars, labels = NULL, recodes = NULL) { # nolint start
  ...
  error_if_items(
    ...
    "Variables in `vars` must be valid aliases in aliases(allVariables(dataset)). This is not true for {items}.", # nolint
  ...
  error_if_items(
    aliases(ds_vars)[!(types(ds_vars) %in% c("categorical", "multiple_response"))],
    "Variables in `vars` must be of type categorical or multiple_response. This is not true for {items}.", # nolint
    and = TRUE, quotes = TRUE
  )

  variables <- list()
  for (i in seq_len(length(ds_vars))) {
    var <- ds_vars[[i]]
    alias <- alias(var)
...
recode_categories <- function(alias, responses, recodes) {
  error_if_items(
    ...
    paste0(
      "Responses in `recodes` must be included in variable responses.",
      "This is not true for {items} in '",
      alias, "'."
    ),
    quotes = TRUE,
    and = TRUE

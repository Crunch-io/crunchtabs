...
#' @param first_col_width width of the first column. Defaults to 20pt.
...
writeExcel <- function(x, filename = NULL, proportions = FALSE, digits = 0, title = getName(x),
    subtitle = NULL, date = Sys.Date(), returndata = TRUE, table_of_contents = FALSE,
...
writeExcel.Toplines <- function(x, filename = NULL, proportions = FALSE, digits = 0,
    ...
    moe = NULL, sample_desc = "", field_period = "", font = "Calibri", font_size = 12,
    append_text = "", min_cell_size = NULL, min_cell_label = NULL, one_per_sheet = TRUE,
    first_col_width = 20, ...) {
...
writeExcel.Crosstabs <- function(x, filename = NULL, proportions = TRUE, digits = 0,
    ...
    banner <- x$banner
    x$results <- reformatResults(x$results, banner, proportions = proportions, digits = digits,
...
create_table_of_contents <- function(wb, title, subtitle, sample_desc, field_period,
    ...
    addWorksheet(wb, toc_sheet)
    toc_row <- 1
    writeData(wb, toc_sheet, title, startCol = 1, startRow = toc_row)
    addStyle(wb, toc_sheet, createStyle(textDecoration = "bold"), rows = toc_row,
        cols = 1)
    toc_row <- toc_row + 1
    if (!is.null(subtitle)) {
        writeData(wb, toc_sheet, subtitle, startCol = 1, startRow = toc_row)
        toc_row <- toc_row + 1
    }
    if (sample_desc != "") {
        writeData(wb, toc_sheet, "Sample", startCol = 1, startRow = toc_row)
        writeData(wb, toc_sheet, sample_desc, startCol = 2, startRow = toc_row)
        toc_row <- toc_row + 1
    }
    if (field_period != "") {
        writeData(wb, toc_sheet, "Conducted", startCol = 1, startRow = toc_row)
        writeData(wb, toc_sheet, field_period, startCol = 2, startRow = toc_row)
        toc_row <- toc_row + 1
    }
    if (!is.null(moe)) {
        writeData(wb, toc_sheet, "Margin of Error", startCol = 1, startRow = toc_row)
        writeData(wb, toc_sheet, moe, startCol = 2, startRow = toc_row)
        toc_row <- toc_row + 1
    }
    toc_row <- toc_row + 1
    writeData(wb, toc_sheet, "List of Tables", startCol = 1, startRow = toc_row)
    toc_row <- toc_row + 1
...
writeExcelVarBanner <- function(wb, ws, x, banner, start_col = 1, start_row = 1,
    ...
    lapply(seq_along(banner), function(bidx) {
        writeData(wb, ws, getName(banner[[bidx]]), startCol = multicols_csum[bidx],
            startRow = crow)
        mergeCells(wb, ws, cols = multicols_csum[bidx]:(multicols_csum[bidx + 1] -
            1), rows = crow)
        addStyle(wb, ws, createStyle(textDecoration = "bold", halign = "center"),
    ...
    writeData(wb, ws, df, startCol = ccol, startRow = crow, rowNames = TRUE)
    addStyle(wb, ws, createStyle(halign = "center"), rows = crow, cols = multicols_csum[[1]]:(multicols_csum[[length(multicols_csum)]] -
        1), stack = TRUE)
    addStyle(wb, ws, createStyle(textDecoration = "italic", halign = "center"), rows = crow +
        nrow(df), cols = multicols_csum[[1]]:(multicols_csum[[length(multicols_csum)]] -
        1), stack = TRUE)
    addStyle(wb, ws, createStyle(border = "Bottom"), rows = crow, cols = 1:(multicols_csum[[length(multicols_csum)]] -
        1), stack = TRUE)
    addStyle(wb, ws, createStyle(border = "Top"), rows = crow + nrow(df) - ifelse(show_totals,
        1, 0), cols = 1:(multicols_csum[[length(multicols_csum)]] - 1), stack = TRUE)
    addStyle(wb, ws, createStyle(numFmt = paste0("0", if (digits > 0)
    ...
    setColWidths(wb, ws, 1, first_col_width)
...
writeExcelVarTopline.ToplineCategoricalArray <- function(wb, ws, x, start_col = 1,
    ...
    df <- as.matrix(x$data)
    writeData(wb, ws, df, startCol = start_col, startRow = start_row, rowNames = TRUE,
        colNames = TRUE)
    ...
    addStyle(wb, ws, createStyle(numFmt = numFmt), rows = rows, cols = cols, gridExpand = TRUE,
        stack = TRUE)
    return(drows)
}
...
writeExcelVarTopline.ToplineNumeric <- function(wb, ws, x, start_col = 1, start_row = 1,
    ...
    addStyle(wb, ws, createStyle(numFmt = numFmt), rows = rows, cols = 2, gridExpand = FALSE,
        stack = TRUE)
    return(drows)
}
...
writeVarHeader <- function(wb, ws, x, start_col = 1, start_row = 1, toc_sheet = NULL,
    ...
    crow <- start_row
    ccol <- start_col
    writeData(wb, ws, getName(x), startCol = ccol, startRow = crow)
    addStyle(wb, ws, createStyle(textDecoration = "bold"), rows = crow, cols = ccol)
    crow <- crow + 1
    if (!is.null(toc_sheet)) {
        writeFormula(wb, toc_sheet, startRow = toc_row, x = makeHyperlinkString(sheet = ws,
            row = start_row, col = start_col, text = getName(x)))
        addStyle(wb, toc_sheet, createStyle(fontColour = "black", textDecoration = "underline"),
            rows = toc_row, cols = 1, stack = FALSE)
    }
    writeData(wb, ws, getDescription(x), startCol = ccol, startRow = crow)
    crow <- crow + 1
    filtertext <- getNotes(x)
    if (filtertext != "") {
        writeData(wb, ws, filtertext, startCol = ccol, startRow = crow)
        addStyle(wb, ws, createStyle(textDecoration = "italic"), rows = crow, cols = ccol)
        crow <- crow + 1
    }
    crow + 1
}
...
writeVarGeneral <- function(x, banner = NULL, filename = NULL, proportions = TRUE,
    ...
    modifyBaseFont(wb, fontSize = font_size, fontColour = "black", fontName = font)
    ...
    if (append_text != "") {
        ...
        addWorksheet(wb, worksheet_name)
        writeData(wb, worksheet_name, append_text, startCol = 1, startRow = 1)
    }
    if (!is.null(filename)) {
...
# 2) addStyle(wb, ws, createStyle(numFmt = numFmt), rows = rows, cols = cols,
# gridExpand = is_grid, stack = TRUE) setColWidths(wb, ws, 1, first_col_width)
# return(crow + drows) }

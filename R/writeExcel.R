...
#' @export
writeExcel <- function(
  data_summary, filename = getName(data_summary), wb = NULL, theme = themeDefaultExcel(),
  ...
  if (!all(n_or_percent %in% c("counts", "percents"))) {
    stop("`n_or_percent` must be a character vector containing 'counts', 'percents' or both, not ",
         paste0("'", n_or_percent, "'", collapse = ", "))
  }
  ...
  return(writeReportGeneral(
    data_summary = data_summary, banner = data_summary$banner, filename = filename,
    wb = wb, theme = theme, title = title, subtitle = subtitle,
    table_of_contents = table_of_contents, n_or_percent = n_or_percent,
...
create_styles <- function(theme) {
  find_null_or_base <- function(theme, info_name, elem) {
    ...
    if (is.null(x) || (!is.null(theme[[elem]]) && x %in% theme[[elem]])) {
      return(NULL)
    }
    return(x)
  }

  numFmt <- paste0("0", if (theme$digits > 0) paste0(
    ".", paste0(rep(0, theme$digits), collapse = "")))
  ...
  style_list <- sapply(grep("^format_", names(theme), value = TRUE), function(v) {
    if (!is.null(theme[[v]])) {
      ...
      openxlsx::createStyle(
        fontName = find_null_or_base(theme, v, "font"),
        ...
        numFmt = if (v %in% c("format_weighted_n", "format_unweighted_n")) {
          "0"
        } else {
          "GENERAL"
        }
  ...
  style_list$body_counts <- openxlsx::createStyle(
    numFmt = numFmt, halign = theme$halign, valign = theme$valign)
  style_list$body_proportions <- openxlsx::createStyle(
    numFmt = numFmtProp, halign = theme$halign, valign = theme$valign)
  style_list$split_border <- openxlsx::createStyle(
    border = if (!is.null(theme$format_banner_split$empty_col) &&
                 theme$format_banner_split$empty_col)
      "LeftRight" else "left",
    borderStyle = if (!is.null(theme$format_banner_split$border_style))
      theme$format_banner_split$border_style else "None",
...
write_and_style <- function(wb, ws, data, style, start_row, cols, write_as_rows) {
  ...
  data <- data.frame(lapply(data, function(d) {
    tryCatch(as.numeric(as.character(d)), warning = function(w) {
      return(d)
    })
  }))
  openxlsx::writeData(wb, ws, data, startCol = min(cols), startRow = start_row, colNames = FALSE)
  openxlsx::addStyle(wb, ws, style, rows = start_row:(start_row + nrow(data) - 1),
                     cols = cols, stack = TRUE, gridExpand = TRUE)
...
write_report_desc <- function(wb, ws, theme, styles, title, subtitle) {
  ...
  if (!is.null(theme$format_title) && !is.null(title)) {
    start_row <- write_and_style(wb, ws, title, styles$format_title, start_row = start_row,
                                 cols = 1, write_as_rows = TRUE)
  }
  if (!is.null(theme$format_subtitle) && !is.null(subtitle)) {
    start_row <- write_and_style(wb, ws, subtitle, styles$format_subtitle, start_row = start_row,
                                 cols = 1, write_as_rows = TRUE)
  }
  return(start_row + 1)
}
...
write_banner_panel <- function(wb, ws, theme, styles, banner, title, subtitle,
  ...
  start_row <- sr <- write_report_desc(
    wb = wb, ws = ws, theme = theme, styles = styles,
  ...
  if (!is.null(banner)) {
    if (!is.null(theme$format_banner_names)) {
      data <- unlist(sapply(seq_along(banner), function(bv) {
        c(getName(banner[[bv]]),
          rep("", times = length(banner_info$multicols[[bv]]) - 1 + banner_info$empty_col))
      }))
      start_row <- write_and_style(
        wb, ws, data = data,
        style = styles$format_banner_names, start_row = start_row,
        cols = banner_info$format_cols, write_as_rows = FALSE)
      lapply(setdiff(seq_along(banner_info$multicols), 1), function(bv) {
        openxlsx::mergeCells(
          wb, ws,
          cols = banner_info$multicols_csum[bv]:(banner_info$multicols_csum[bv + 1] -
          1 - banner_info$empty_col), rows = start_row - 1)
      })
    }
    start_row <- write_and_style(wb, ws,
      data = getItemData(banner, "categories", banner_info$empty_col, round = FALSE),
      style = styles$format_banner_categories, start_row = start_row,
      cols = banner_info$format_cols, write_as_rows = FALSE
    )
    if (!is.null(theme$format_weighted_n) && theme$format_weighted_n$position_fixed) {
      start_row <- write_and_style(wb, ws,
        data = c(theme$format_weighted_n$name, getItemData(
          banner, "weighted_n", banner_info$empty_col, round = TRUE)),
        style = styles$format_weighted_n, start_row = start_row,
        cols = c(1, banner_info$format_cols), write_as_rows = FALSE
      )
    }
    if (!is.null(theme$format_unweighted_n) && theme$format_unweighted_n$position_fixed) {
      start_row <- write_and_style(wb, ws,
        data = c(theme$format_unweighted_n$name,
                 getItemData(banner, "unweighted_n", banner_info$empty_col, round = TRUE)),
        style = styles$format_unweighted_n, start_row = start_row,
        cols = c(1, banner_info$format_cols), write_as_rows = FALSE
      )
    }
    if (percent_row) {
      ...
      start_row <- write_and_style(wb, ws, data = data,
                                   style = styles$format_banner_categories, start_row = start_row,
                                   cols = banner_info$format_cols, write_as_rows = FALSE)
      openxlsx::addStyle(wb, ws, openxlsx::createStyle(textDecoration = NULL),
                         rows = start_row - 1, cols = banner_info$format_cols, stack = TRUE)
    }
    openxlsx::addStyle(wb, ws, styles$format_totals_column,
                       rows = sr:(start_row - 1), cols = 2, stack = TRUE)
    openxlsx::addStyle(wb, ws, styles$split_border,
      rows = sr:(start_row - 1),
  ...
  openxlsx::addStyle(wb, ws, styles$format_label_column, rows = sr:(start_row - 1),
                     cols = 1, gridExpand = FALSE, stack = TRUE)

  openxlsx::pageSetup(wb, ws, printTitleRows = (1 + if (!is.null(theme$format_title)) {
    length(title)
  } else {
    0
  }):start_row)
  openxlsx::freezePane(wb, ws, firstActiveRow = start_row,
                       firstActiveCol = theme$excel_freeze_column + 1)
...
write_var_header <- function(wb, ws, var, theme, styles, start_row, toc_sheet, toc_row, toc_col) {
  ...
  if (!is.null(toc_sheet)) {
    openxlsx::writeFormula(wb, toc_sheet,
      startCol = toc_col, startRow = toc_row,
    ...
    openxlsx::writeData(
      wb,
      toc_sheet,
      paste0(c(var_info$format_var_subname, var_info$format_var_description), collapse = " - "),
      startCol = toc_col + 1, startRow = toc_row
    )
  }
  for (info_name in names(var_info)) {
    start_row <- write_and_style(
      wb, ws, data = var_info[[info_name]],
      style = styles[[info_name]], start_row = start_row, cols = 1, write_as_rows = FALSE)
  }
  return(start_row)
}

writeExcelVar <- function(wb, ws, theme, styles, banner_name, var, banner_info,
                          start_row, start_col,
                          toc_sheet, toc_row, toc_col, hypothesis_test, proportions) {
  if (getOption("testing_crunchtabs", default = FALSE)) print(var$alias)
  start_row <- sr <- write_var_header(
    wb = wb, ws = ws, var = var, theme = theme, styles = styles,
  ...
  style_if <- function(ifStatement, start_row, style, data, cols) {
    if (ifStatement) {
      ...
      for (s in style) {
        if (!is.null(s)) {
          openxlsx::addStyle(
            wb = wb, sheet = ws, style = s, rows = start_row:(start_row + nr - 1),
  ...
  var_info <- reformatVar(
    var = var, banner_name = banner_name, theme = theme,
  ...
  openxlsx::writeData(
    wb = wb, sheet = ws, x = all_data, startCol = 2, startRow = start_row, colNames = topline_array,
    headerStyle = styles$format_banner_categories,
    borders = if (!is.null(theme$excel_table_border$border_style)) {
      "surrounding"
    } else {
      "none"
    },
    borderColour = theme$excel_table_border$border_color,
    borderStyle = if (!is.null(theme$excel_table_border$border_style)) {
      theme$excel_table_border$border_style
    } else {
      "thin"
    }
  )
  ...
  style_if(!is.null(styles$format_totals_column), start_row, styles$format_totals_column,
           all_data, cols = 2)
  ...
  min_row <- (start_row - if (is.null(var_info$min_cell_top)) {
    0
  } else {
    nrow(var_info$min_cell_top)
  })
  if (!is.null(theme$format_min_base$mask)) {
    for (bki in which(colSums(min_cell_mask, na.rm = TRUE) != 0)) {
      ...
      for (bkj in seq_along(vals$lengths)) {
        if (!is.na(vals$values[bkj]) && vals$values[bkj]) {
          write_and_style(
            wb = wb, ws = ws, data = rep(theme$format_min_base$mask, vals$lengths[bkj]),
            style = styles$format_min_base, cols = bki + 1, start_row = vals$start[bkj],
            write_as_rows = TRUE
          )
        }
      }
    ...
    rows <- rows + min_row - 1
    openxlsx::addStyle(wb = wb, sheet = ws, style = styles$format_min_base, rows = rows,
                       cols = cols, stack = TRUE)
  }

  if (any(var_info$inserts %in% "Heading")) {
    openxlsx::addStyle(wb, ws, styles$format_headers,
      rows = (start_row + which(var_info$inserts %in% "Heading") - 1),
      cols = c(1, banner_info$format_cols), gridExpand = TRUE, stack = TRUE
    )
  }
  if (any(var_info$inserts %in% "Subtotal")) {
    openxlsx::addStyle(wb, ws, styles$format_subtotals,
      rows = (start_row + which(var_info$inserts %in% "Subtotal") - 1),
      cols = c(1, banner_info$format_cols), gridExpand = TRUE, stack = TRUE
    )
  }
  ...
  write_and_style(wb, ws,
    data = unlist(lapply(data_list[var_info$data_order], rownames)),
...
hypho_test <- function(wb, ws, cross_tab_var, banner_name, margin, banner_info, styles,
  ...
  if (sum(dim(pvals_row)) == 0) {
    return(NULL)
  }
  for (pvi in seq_along(pvals)) {
    inds <- which(!is.na(pvals_row) & pvals_row < pvals[pvi] & pvi < length(pvals) &
                    pvals_row >= pvals[pvi + 1] |
                    (pvi == length(pvals) & pvals_row < pvals[pvi] & pvals_row > 0), arr.ind = TRUE)
    openxlsx::addStyle(wb, ws, styles[[pcol_pos[pvi]]],
      rows = crow + inds[, 1] - 1,
      cols = ccol + inds[, 2] - 1, gridExpand = FALSE, stack = TRUE
    )
  }
  pvals <- pvals * (-1) # nolint
  for (pvi in seq_along(pvals)) {}

  inds <- which(!is.na(pvals_row) & pvals_row > pvals[pvi] & pvi < length(pvals) &
                  pvals_row <= pvals[pvi + 1] | (pvi == length(pvals) & pvals_row > pvals[pvi] &
                                                   pvals_row < 0), arr.ind = TRUE)
  openxlsx::addStyle(wb, ws, styles[[pcol_neg[pvi]]],
    rows = crow + inds[, 1] - 1,
    cols = ccol + inds[, 2] - 1, gridExpand = FALSE, stack = TRUE
  )
  # nocov end
...
writeReportGeneral <- function(data_summary, banner, filename, wb, theme,
  ...
  openxlsx::modifyBaseFont(wb, fontSize = theme$font_size, fontColour = theme$font_color,
                           fontName = theme$font)
  ...
  if (table_of_contents) {
    openxlsx::addWorksheet(wb, toc_sheet, gridLines = theme$excel_show_grid_lines,
                           header = theme$excel_header, footer = theme$excel_footer,
                           orientation = theme$excel_orientation)
    toc_row <- toc_start_row <- write_report_desc(
      wb = wb, ws = toc_sheet, theme = theme, styles = styles,
      title = title, subtitle = subtitle
    )
  ...
  banner_names <- if (is.null(banner))
    "Results" else names(banner)
  worksheet_names <- sapply(banner_names, function(bn) {
    ws_names <- if (theme$one_per_sheet)
      names(data_summary$results) else bn
    ws_names[nchar(ws_names) > 25] <- paste0(strtrim(ws_names[nchar(ws_names) > 25], 25),
                                             which(nchar(ws_names) > 25))
    if (theme$one_per_sheet && length(banner_names) > 1)
      ws_names <- paste0(ws_names, "_", which(banner_names %in% bn))
    return(ws_names)
  }, simplify = FALSE)
  ...
  for (worksheet_name in make_worksheets) {
    openxlsx::addWorksheet(wb, worksheet_name,
                           gridLines = theme$excel_show_grid_lines, header = theme$excel_header,
                           footer = theme$excel_footer, orientation = theme$excel_orientation)
    openxlsx::setColWidths(wb, worksheet_name, cols = 1, theme$format_label_column$col_width)
  }

  if (length(n_or_percent) == 2) {
  ...
  for (bix in seq_along(banner_names)) {
    ...
    if (table_of_contents) {
      openxlsx::writeData(
        wb, toc_sheet, paste0(banner_name, bna), startRow = toc_start_row, startCol = toc_col)
    }
    if (!theme$one_per_sheet) {
      worksheet_name <- paste0(worksheet_names[[banner_name]], bna)
      last_row_used <- write_banner_panel(
        wb = wb, ws = worksheet_name, theme = theme, styles = styles,
    ...
    for (vidx in seq_along(data_summary$results)) {
      ...
      last_row_used <- writeExcelVar(
        wb = wb, ws = worksheet_name, var = data_summary$results[[vidx]], theme = theme,
        styles = styles, banner_name = banner_name, banner_info = banner_info,
        start_row = last_row_used + 1, start_col = 2, toc_sheet = toc_sheet,
        toc_row = toc_row, toc_col = toc_col,
    ...
    if (logging) {
      ...
      print(paste0(end.time, " -- banner generation: ", banner_name, bna, " -- end -- elapsed: ",
                   round(difftime(end.time, start.time, units = "mins"), 2), " mins"))
    }
  }
  if (table_of_contents) {
    openxlsx::addStyle(wb, toc_sheet, styles$format_banner_categories, rows = toc_start_row,
                       cols = 1:(toc_col - 1), stack = FALSE)
  }
  ...
  if (logging) {
    ...
    print(paste(end.time, "-- workbook save -- end -- elapsed: ",
                round(difftime(end.time, start.time, units = "mins"), 2), "mins"))
    print(paste(end.time, "-- workbook generation -- end -- elapsed: ",
                round(difftime(end.time, start.time.wb, units = "mins"), 2), "mins"))
  }

...
test_that("Write Excel file", {
    ...
    expect_silent(wb <- openxlsx::loadWorkbook("Example Dataset with Nets.xlsx"))
    
    expect_equal(openxlsx::getSheetNames("Example Dataset with Nets.xlsx"), 
        c("RESULTS_P", "RESULTS_C", "R2_P", "R2_C"))
    expect_equal(openxlsx::getBaseFont(wb)$size$val, "12")

...
test_that("Creates result object appropriately for a TextVariable", {
  ...
  mockery::stub(
    nonTabBookSummary.DatetimeVariable, "as.vector",
    structure(c(
  ...
  expect_equal(
    ...
    structure(list(
      ...
      data_order = c("body", weighted_n = "weighted_n"), inserts = c(
        ...
        weighted_n = structure(list(20L), .Names = NA_character_,
                               class = "data.frame", row.names = "Weighted N")
      ),
...
test_that("Creates result object appropriately for a TextVariable", {
  ds <- readRDS(test_path("fixtures/example_dataset.rds"))
  mockery::stub(
    nonTabBookSummary.NumericVariable, "as.vector",
...
test_that("Creates result object appropriately for a TextVariable", {
  ...
  expect_equal(
    res$rownames,
    c(
      "Ali", "Clyde", "Fluffy", "Hugo", "Jasmine", "Lady", "Rocky",
...
test_that("resultsObject on unweighted data", {
  x <- new("TextVariable", filter = new("CrunchLogicalExpr",
    dataset_url = "",
    expression = list(), filter = list()
  ), tuple = new("VariableTuple",
    index_url = "a_long_url",
    entity_url = "a_long_url",
    body = list(
      discarded = FALSE, alias = "q3", secure = FALSE,

...
cats <- new("Categories", .Data = list(new("Category", .Data = list(1L, FALSE, "16 to 24", NULL),
                                           names = c("id", "missing", "name", "numeric_value")),
    new("Category", .Data = list(2L, FALSE, "25 to 34", NULL),
        names = c("id", "missing", "name", "numeric_value")),
    new("Category", .Data = list(3L, FALSE, "35 to 44", NULL),
        names = c("id", "missing", "name", "numeric_value")),
    new("Category", .Data = list(4L, FALSE, "45 to 54", NULL),
        names = c("id", "missing", "name", "numeric_value")),
...
test_that("Error handling - banner", {
  ...
  expect_error(
    ...
    "Variables in `vars` must be valid aliases in aliases(allVariables(dataset)). This is not true for 'a' and 'b'.", # nolint
    fixed = TRUE
  )
  ...
  expect_error(
    banner(ds, list(Results = c("art2")), labels = list("Doggy")),
    "`labels` must be a named list or vector."
  )
  expect_error(
    banner(ds, list(Results = c("art3")), labels = c(allpets1 = "All Pets")),
    "Variables in `labels` must be included in `vars`. This is not true for 'allpets1'."
  )
  expect_error(
    banner(ds, list(Results = c("art3", "age4")), labels = c(allpets1 = "All Pets",
                                                             age4 = "Age (2 categories")),
    "Variables in `labels` must be included in `vars`. This is not true for 'allpets1'."
  )
  expect_error(
    banner(ds, list(Results = c("art2")), recodes = list(list("Dog" = "Doggy"))),
    "`recodes` must be a named list."
  )
  expect_error(
    banner(ds, list(Results = c("art2")), recodes = list(favpet1 = list("Dog" = "Doggy"))),
    "Variables in `recodes` must be included in `vars`. This is not true for 'favpet1'."
  )
  expect_error(
    banner(ds, list(Results = c("art2", "age4")), recodes = list(
      age4 = list("16 to 34" = "<= 34"), favpet1 = list("Dog" = "Doggy"))),
    "Variables in `recodes` must be included in `vars`. This is not true for 'favpet1'."
  )
  expect_error(
    banner(ds, list(Results = c("art2")), recodes = list("art2" = c("Dog" = "Doggy"))),
    "`recodes` must be a list of lists. This is not true for art2."
  )
  expect_error(
    banner(ds, list(Results = c("movies5"))),
    "Variables in `vars` must be of type categorical or multiple_response. This is not true for 'movies5'."  # nolint
  )
  expect_error(
    banner(ds, list(Results = c("movies5", "tv4", "movies3", "age"))),
    "Variables in `vars` must be of type categorical or multiple_response. This is not true for 'movies5', 'tv4', 'movies3', and 'age'."  # nolint
  )
  expect_error(
    banner(ds, list(Results = c("art3", "movies5", "movies3", "age4"))),
    "Variables in `vars` must be of type categorical or multiple_response. This is not true for 'movies5' and 'movies3'."  # nolint
  )
})
...
test_that("Single banner with single variable", {
  ...
  expect_named(banner_data[["Results"]][["___total___"]],
               c("alias", "name", "type", "old_categories", "categories_out", "categories"))
  expect_named(banner_data[["Results"]][["age5"]],
               c("alias", "name", "type", "old_categories", "categories_out", "categories"))
  ...
  expect_identical(banner_data[["Results"]][["age5"]][["categories_out"]],
                   c("16 to 24", "25 to 34", "35 to 44", "45 to 54", "55+"))
  expect_identical(banner_data[["Results"]][["age5"]][["categories"]],
                   c("16 to 24", "25 to 34", "35 to 44", "45 to 54", "55+"))
})
...
test_that("Single banner with two variables", {
  ...
  expect_named(banner_data[["Results"]][["age5"]],
               c("alias", "name", "type", "old_categories", "categories_out", "categories"))
  expect_named(banner_data[["Results"]][["art3"]],
               c("alias", "name", "type", "old_categories", "categories_out", "categories"))
  expect_identical(banner_data[["Results"]][["age5"]][["alias"]], "age5")
  expect_identical(banner_data[["Results"]][["age5"]][["name"]], "Age 5")
  ...
  expect_identical(banner_data[["Results"]][["age5"]][["categories"]],
                   c("16 to 24", "25 to 34", "35 to 44", "45 to 54", "55+"))
  expect_identical(banner_data[["Results"]][["art3"]][["old_categories"]],
                   c("Painting", "Sculpture", "Dance", "Music", "Poetry"))
  expect_identical(banner_data[["Results"]][["art3"]][["categories_out"]],
                   c("Painting", "Sculpture", "Dance", "Music", "Poetry"))
  expect_identical(banner_data[["Results"]][["art3"]][["categories"]],
                   c("Painting", "Sculpture", "Dance", "Music", "Poetry"))
})
...
test_that("Single banner with one variable, recodes - categories rename", {
  ...
  expect_identical(banner_data[["Results"]][["age5"]][["old_categories"]],
                   c("16 to 24", "25 to 34", "35 to 44", "45 to 54", "55+"))
  expect_identical(banner_data[["Results"]][["age5"]][["categories_out"]],
                   c("Under 25", "25 to 34", "35 to 44", "45 to 54", "Over 54"))
  expect_identical(banner_data[["Results"]][["age5"]][["categories"]],
                   c("Under 25", "25 to 34", "35 to 44", "45 to 54", "Over 54"))
})
...
test_that("Single banner with one variable, recodes - categories rename, hiding", {
  ...
  expect_identical(banner_data[["Results"]][["age5"]][["old_categories"]],
                   c("16 to 24", "25 to 34", "35 to 44", "45 to 54", "55+"))
  expect_identical(banner_data[["Results"]][["age5"]][["categories_out"]],
                   c("Under 25", "25 to 34", "35 to 44", NA, NA))
  expect_identical(banner_data[["Results"]][["age5"]][["categories"]],
                   c("Under 25", "25 to 34", "35 to 44"))
})
...
test_that("Adjustments for subtotals", {
  ...
  expect_equal(
    ...
    list(
      inserts_obj = structure(list(), .Names = character(0)),

...
test_that("Error handling - banner", {
  ...
  expect_error(
    banner(ds, list(Results = c("art3", "age4")), labels = c(allpets1 = "All Pets", age4 = "Age (2 categories")),
    "Variables in `labels` must be included in `vars`. This is not true for 'allpets1'."
  )
  expect_error(
  ...
  expect_error(
    banner(ds, list(Results = c("art2", "age4")), recodes = list(age4 = list("16 to 34" = "<= 34"), favpet1 = list("Dog" = "Doggy"))),
    "Variables in `recodes` must be included in `vars`. This is not true for 'favpet1'."
  )
  expect_error(
...
test_that("Single banner with one variable, recodes - categories rename", {
  banner_data <- with_mock(subvariables = subvariables_over, categories = categories_over, banner(ds,
    vars = list(Results = c("age5")),
